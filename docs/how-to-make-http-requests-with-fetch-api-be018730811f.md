# 如何使用 Fetch API 发出 HTTP 请求并在 React 应用程序中使用 Fetch

> 原文：<https://levelup.gitconnected.com/how-to-make-http-requests-with-fetch-api-be018730811f>

![](img/bf6d3bee5c1ee4da4e15cc0bc4588abe.png)

**订阅** [**我的邮箱列表**](http://jauyeung.net/subscribe/) **或者关注我的** [**推特**](https://twitter.com/AuMayeung)

Fetch API 是大多数现代浏览器中内置的 web 标准，允许我们向服务器发出 HTTP 请求。使用 Fetch API 从调用`fetch()`函数开始，这允许我们用标准的 HTTP 动词发出 HTTP 请求:`GET`、`POST`、`PUT`、`PATCH`和`DELETE`。`fetch`函数返回一个承诺，该承诺在请求完成时解决。下面是一个使用`fetch`的基本例子:
...
既然`fetch`返回一个承诺，我们也可以用`async` / `await`:

```
async () =>{
  const response = await fetch('http://example.com/example.json')
  return response.json();
}
```

上面的代码从服务器接收 JSON 响应，然后将响应转换成 JavaScript 对象。注意，`json`函数也返回一个承诺。

现代浏览器中实现了对 Fetch API 的浏览器支持，因此我们可以使用它来编写我们的应用程序。

## 用获取和反应构建一个简单的 Web 应用程序

在这个故事中，我们将构建一个地址簿应用程序。首先，我们需要运行 Create React App 来搭建应用程序。我们执行`npx create-react-app address-book`来创建包含初始文件的 app 项目文件夹。

该应用程序将有一个主页来显示联系人，并让我们打开一个模式来添加联系人。将有一个表，显示所有的联系人和编辑和删除按钮，每一行编辑或删除每个联系人。

对于简单的状态管理，我们使用 MobX。它允许 up 设置一个存储，该存储可以传播到观察存储变化的任何组件。

为了安装库，我们运行`npm i bootstrap formik react-bootstrap mobx mobx-react react-router-dom yup`。`react-router-dom`是 React 路由器最新版本的包名。

现在我们已经安装了所有的库，我们可以开始构建应用程序了。除非另有说明，否则所有文件都在`src`文件夹中。首先，我们在 MobX 商店上工作。我们在`src`文件夹中创建一个名为`store.js`的文件，并添加以下内容:
...
这是一个简单的存储，为整个应用程序保存联系人的数组。`setContacts`函数让我们从任何组件中设置联系人，在那里我们传递 store 对象。
...
将`ContactsStore`中的`contacts`数组指定为可被组件观察变化的实体。`setContacts`功能被指定为可用于设置存储中的`contacts`数组的功能。

现在让我们编写我们的`fetch`函数，稍后将在应用程序中使用。我们制作一个名为`requests.js`的文件，并添加:

```
const APIURL = "[http://localhost:3000](http://localhost:3000)";export const getContacts = async () => {
...
我们在这里使用 Fetch API 来发出 HTTP 请求。要使用`fetch`，如果我们发送请求数据，我们必须指定请求有效载荷的数据类型。例如在`addContact`和`editContact`函数中，我们指定`“Content-Type”: “application/json”`作为发送 JSON 的`headers`。此外，我们必须将主体字符串化以发送到服务器。`fetch`返回一个承诺，所以我们可以使用`async` / `await`。在响应中，我们必须调用`json`将响应体转换成 JSON，这样我们就可以在其他地方使用它。此外,`mode`必须设置为`cors`,以便我们进行跨域通信。
...
我们可以将商店传递给任何组件，比如`HomePage`，它会将组件传递给`ContactForm`。
...
对于表单验证，我们使用 Formik 来简化联系人表单的构建。我们使用嵌套在`Formik`组件中的 Boostrap `Form`组件，这样我们就可以使用 Formik 的`handleChange`、`handleSubmit`、`values`、`touched`和`errors`参数。

`handleChange`是一个函数，让我们不用自己写代码就可以从输入中更新表单域数据。`handleSubmit`是我们传递给`Formik`组件的`onSubmit`处理程序的函数。函数中的参数是我们输入的数据，字段名作为键，由每个字段的`name`属性定义，每个字段的值作为那些键的值。注意，在每个`value`属性中，我们有`||''`，这样我们就不会得到未定义的值，并防止触发不受控制的表单警告。
...
请注意，我们使用了`export default observer(HomePage);`而不是`export default HomePage;`。我们需要将`HomePage`包装在`observer`函数调用中，这样来自商店的最新数据将被传播到这个组件中。

它有显示联系人的表格和添加、编辑和删除联系人的按钮。它在第一次加载时用在`useEffect`的回调函数中调用的`getData`函数获取数据。每次渲染都会调用`useEffect`的回调，所以我们想要设置一个`initialized`标志，并检查它是否只在`true`时才加载。
...
![](img/9d3348846e2374867d8616963dcd66e0.png)![](img/f0189119955e2e23d8ca9a28cc9761f9.png)![](img/751d2681035b39a9bbd8da8b261ec55b.png)
...
我将这个契约复制到 Remix 中的一个新文件中，编译它，并部署它。应该没有错误和警告，所以我转到`Run` 选项卡并点击`Deploy`。
...
1.  **考虑一个带有两个索引参数* `*t1*` *和* `*t2*` *的事件，按此顺序调用一个带有多个参数的函数，其中* `*a1*` *在* `*a2*`之前*
...
*Alexandre Pinto——Artos(Aventus 生态系统党)的区块链开发人员*

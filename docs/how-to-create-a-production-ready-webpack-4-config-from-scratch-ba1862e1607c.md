...
> 原文：<https://levelup.gitconnected.com/how-to-create-a-production-ready-webpack-4-config-from-scratch-ba1862e1607c>

![](img/fdd01ddd1d3e92c674359c839b3e3484.png)
...
![](img/f2eb619ffde5b9fdf223097fb26ad2b5.png)
...
如果您想继续学习，本文中的所有代码都可以在 GitHub 中找到。[起点在这里](https://github.com/thawkin3/webpack-training-1/tree/demo/start)，完成结果在这里[。](https://github.com/thawkin3/webpack-training-1)
...
`script`标签引用了我们的`./src/index.js`文件，其中只有几行 JavaScript 输出文本“Hello from webpack！”：
...
如果你把`index.html`文件拖到你的浏览器中，你应该能看到我们简单的网页:

![](img/be7b69f62c6370d1ce63250c0bfa685b.png)

演示应用程序输出 1 —来自 webpack 的 hello
...
现在，您应该看到在您的项目目录中创建了一个`dist`目录。在它里面你应该会看到一个`main.js`文件，这是我们的简化代码。
...
现在，在浏览器中刷新页面，您仍然应该看到完全相同的输出，只是这次是“来自 webpack 的 Hello！”文本现在由`./dist/main.js`文件生成。

![](img/fbe04b160ab45892958ef60e63e17fa5.png)

演示应用程序输出 2 —无变化

# 创建 Webpack 配置文件

好了，现在我们已经安装了 webpack 并完成了快速的健全性检查，让我们创建一个实际的 webpack 配置文件。我们将创建一个名为`webpack.config.js`的文件，并将以下代码放入其中:
...
现在我们可以用命令`yarn build`运行我们的构建过程。继续运行该命令，验证您已经正确设置了所有内容。您甚至可以在运行`yarn build`命令之前删除您的`dist`目录，以验证目录正在生成。
...
现在再次运行`yarn build`生成输出。现在您应该会在您的`dist`目录中看到一个`tacos.js`文件。
...
![](img/5f42b856ad9dfb56a257e78292bc3e6c.png)

照片由 [Feelfarbig 杂志](https://unsplash.com/@feelfarbig?utm_source=ghost&utm_medium=referral&utm_campaign=api-credit) / [Unsplash](https://unsplash.com/?utm_source=ghost&utm_medium=referral&utm_campaign=api-credit) 拍摄
...
![](img/4562d1060443e961432f86b53bf76499.png)

照片由[诚信公司](https://unsplash.com/@honest?utm_source=ghost&utm_medium=referral&utm_campaign=api-credit) / [Unsplash](https://unsplash.com/?utm_source=ghost&utm_medium=referral&utm_campaign=api-credit) 拍摄
...
要使用它，我们只需在我们的`webpack.config.js`文件中添加`require`插件，然后将它包含在我们的配置设置中的`plugins`数组中:
...
现在再次运行`yarn build`，您应该在`dist`目录中只看到一个输出文件。问题解决了！

# HTMLWebpackPlugin

![](img/532e0869b83d002d0af87bfd4c3f58c1.png)

弗洛里安·奥利沃 / [昂斯普什](https://unsplash.com/?utm_source=ghost&utm_medium=referral&utm_campaign=api-credit)拍摄的照片
...
有一个插件可以做到这一点！我们可以使用 HTMLWebpackPlugin 来帮助我们管理 HTML 文件。现在让我们将它安装到我们的项目中:

```
yarn add --dev html-webpack-plugin
```

好了，现在让我们把我们的`index.html`文件移到我们的`src`目录中，这样它就是`index.js`文件的兄弟。
...
现在让我们在`webpack.config.js`文件中`require`这个插件，然后在配置设置中将它包含在`plugins`数组中，就像我们对第一个插件所做的那样:
...
在那些用于`HtmlWebpackPlugin`的选项中，我们指定`filename`作为我们希望输出文件被调用的名称。
...
![](img/bbfe945730270f25f4a5837456c16db1.png)

照片由格伦·卡斯滕斯-彼得斯 / [Unsplash](https://unsplash.com/?utm_source=ghost&utm_medium=referral&utm_campaign=api-credit) 拍摄

好的，让我们确保一切仍然正常工作。运行`yarn build`，并确认您在`dist`目录中看到两个文件:`index.html`和`main.js`。

如果您仔细查看您的`index.html`文件，您会看到引用的`main.js`文件。

现在，在你的浏览器中打开`./dist/index.html`文件，验证你的页面加载正确。如果您正确地遵循了这些步骤，您的页面应该仍然可以工作:

![](img/688832249c68f048b9078c4a07857c8e.png)

演示应用程序输出 3 —无变化

# 创建开发服务器

![](img/39da5b28f20fd14baa3c79ca16c25565.png)

泰勒·维克 / [Unsplash](https://unsplash.com/?utm_source=ghost&utm_medium=referral&utm_campaign=api-credit) 拍摄的照片

到目前为止，我们已经使用`CleanWebpackPlugin`和`HtmlWebpackPlugin`做了一些很好的改进。由于我们已经做了这些更改，我们每次都必须手动运行`yarn build`命令，以查看我们的应用程序中的新变化。我们也只是在浏览器中查看文件，而不是查看从本地运行的服务器提供的内容。让我们通过创建一个开发服务器来改进我们的流程。
...
这个文件看起来也非常类似于我们的原始配置文件。这里我们已经指定了`mode`是`production`并且我们想要为源映射提供`source-map`选项，它为精简的代码提供单独的源映射文件。
...
最后，运行`yarn start`来启动开发服务器。这将打开`[http://localhost:8080/](http://localhost:8080/)`上的应用程序。不再需要直接查看文件，只需将它们拖入浏览器即可！我们现在有了一个真正的实时开发服务器！

您看到的输出看起来应该和往常一样:

![](img/a3ae1fc8e03bf5ec64ecd18f42fade0c.png)
...
![](img/c27c6865e2ff3bb654f7ab001cd5f6ab.png)

演示应用程序输出 5 —来自开发服务器的问候

# 不要重复自己(干)

![](img/08cb815aeac417edf2bc601cd24e9df2.png)

Tobias Jelskov 在 [Unsplash](https://unsplash.com?utm_source=medium&utm_medium=referral) 上拍摄的照片
...
![](img/7eae43c1eaa354fc9023ff0cf4910361.png)

合并

我们可以使用 [webpack-merge](https://github.com/survivejs/webpack-merge) 插件来管理多个配置文件依赖的共享代码。为此，我们将首先安装软件包:
...
![](img/fd6ca9a20096a4af6c887d6ee92d82dd.png)

由 [Unsplash](https://unsplash.com?utm_source=medium&utm_medium=referral) 上的 [Vadim Sherbakov](https://unsplash.com/@madebyvadim?utm_source=medium&utm_medium=referral) 拍摄的照片
...
![](img/2bf056b6edc9b62e03b9ada4b8dbe256.png)

凯文·布茨 / [Unsplash](https://unsplash.com/?utm_source=ghost&utm_medium=referral&utm_campaign=api-credit) 摄影
...
这里有两个特别有用的加载器:[样式加载器](https://webpack.js.org/loaders/style-loader/)和 [css 加载器](https://webpack.js.org/loaders/css-loader/)。让我们把它们包含在我们的项目中，然后讨论它们是如何工作的。
...
`css-loader`解释并解析您在 JavaScript 中引用的导入 CSS 文件。所以在这种情况下，`css-loader`有助于使这一行工作:
...
让我们通过终止当前进程(如果您仍然让它运行)来重启我们的开发服务器，然后用`yarn start`再次启动它。现在，在网络浏览器中，您应该会在`[https://localhost:8080/](https://localhost:8080/)`上看到:

![](img/12578a961aba6267ce49414f998ccd26.png)
...
在本文中，我们不会讨论其他文件类型的加载器，但是请注意，有一个加载器可以加载任何可以想象的东西！您可以使用[文件加载器](https://webpack.js.org/loaders/file-loader/)或 [url 加载器](https://webpack.js.org/loaders/url-loader/)来加载图像和其他资产。您可以使用 [sass-loader](https://webpack.js.org/loaders/sass-loader/) 来处理将 Sass/SCSS 文件转换为 CSS，然后将该输出传送到`css-loader`和`style-loader`。如果你喜欢的话，Webpack 也可以用 [less-loader](https://webpack.js.org/loaders/less-loader/) 处理更少的文件。
...
而且(你猜对了)，有一个加载器:[通天塔加载器](https://babeljs.io/setup#installation)。

要设置`babel-loader`，我们将遵循上面链接的安装指南中的说明。
...
![](img/bf6878903a2da9966b1a1d000e1ff4ca.png)
...
我们可以使用[minicsextractplugin](https://webpack.js.org/plugins/mini-css-extract-plugin/)为我们生成单独的 CSS 文件，而不是将 CSS 作为`style`标签注入到我们的 HTML 中。我们将在生产配置中使用它，而在开发配置中仍然只使用`style-loader`。
...
现在，在我们的`webpack.config.dev.js`文件中，让我们将刚刚从共享配置中删除的`style-loader`和`css-loader`添加回来:

```
const merge = require('webpack-merge')
const commonConfig = require('./webpack.config.common')module.exports = merge(commonConfig, {
  mode: 'development',
  devtool: 'inline-source-map',
  devServer: {
    contentBase: './dist',
...
![](img/3bd2b796c057d7476759e85bcddf97be.png)
...
那么，这里的问题是什么？想象一下，如果我们的应用程序中使用了一个名为`main.js`的文件。然后，用户访问你的应用程序，他们的浏览器缓存了`main.js`文件。

现在，在以后的某个时间点，你已经为你的应用程序发布了新的代码。`main.js`文件的内容已经改变。但是，当这个用户再次访问你的应用程序时，浏览器发现它需要一个`main.js`文件，注意到它有一个缓存的`main.js`文件，并且只使用缓存的版本。用户没有得到你的新代码！
...
![](img/04df46ce96f905db9acfc77810085308.png)

包含内容哈希的 Webpack 生产构建输出

如果您再次运行`yarn build`并将您的新输出与旧输出进行比较，您会注意到两次的内容散列完全相同。

但是，如果您以任何方式编辑您的`./src/index.js`文件，然后再次运行`yarn build`，您将获得一个新的内容散列，因为内容已经改变了！试试看！
...
现在，如果我们运行`yarn build`，然后检查我们的`dist`目录的内容，我们可以看到产生的 CSS 被缩小了。不错！
...
![](img/088f599891c8038448855845235c838d.png)

照片由[Katya Austin](https://unsplash.com/@katya?utm_source=ghost&utm_medium=referral&utm_campaign=api-credit)/[Unsplash](https://unsplash.com/?utm_source=ghost&utm_medium=referral&utm_campaign=api-credit)拍摄
...
*   `webpack-dev-server`创建一个开发服务器，使本地开发更容易。
...
*   我们可以在文件名中包含内容散列，以帮助缓存破坏和管理我们发布的代码的新版本。
*   我们可以用`optimize-css-assets-webpack-plugin`来缩小我们的 CSS。
*   我们可以用`terser-webpack-plugin`来缩小我们的 JavaScript。

# 下一步是什么？

![](img/f61655f25d21409dab390db9051c6cb6.png)

在本文中，我们已经创建了一个相当不错的 webpack 配置。我们讨论的所有这些技术都是行业标准，在企业级项目中普遍使用。

但是还有更多！其他高级 webpack 主题包括[代码拆分](https://webpack.js.org/guides/code-splitting/)、[懒加载](https://webpack.js.org/guides/lazy-loading/)、[树抖动](https://webpack.js.org/guides/tree-shaking/)等等！

如果你有兴趣自己探索 webpack，我强烈推荐你阅读官方的 webpack 指南。

同样，我们在本教程中学习的所有代码都可以在 GitHub 中找到。[起点在此处](https://github.com/thawkin3/webpack-training-1/tree/demo/start)，完成结果在此处。

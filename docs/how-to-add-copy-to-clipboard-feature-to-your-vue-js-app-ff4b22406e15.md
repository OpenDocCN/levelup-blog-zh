# 如何将复制到剪贴板功能添加到 Vue.js 应用程序

> 原文：<https://levelup.gitconnected.com/how-to-add-copy-to-clipboard-feature-to-your-vue-js-app-ff4b22406e15>

![](img/22b5d837147ef4d51ad21537dbfb17d8.png)

复制到剪贴板功能是像密码管理器这样的 web 应用程序的一个受欢迎的功能，在这里人们不方便高亮显示文本然后复制它。这是一个添加到您自己的 web 应用程序的简单功能。

在本文中，我们将构建一个密码管理器，让您输入、编辑和删除密码，并让他们将自己的用户名和密码复制到剪贴板，以便在他们喜欢的任何地方使用。我们将使用 Vue.js 来构建应用程序。

# 入门指南

首先，我们通过运行`npx @vue/cli create password-manager`来创建项目。在向导中，选择“手动选择功能”，并选择在我们的应用程序中包括 Babel、Vue 路由器和 Vuex。

接下来，我们安装一些我们需要的库。我们需要 Axios 进行 HTTP 请求，Bootstrap Vue 进行样式设计，V-Clipboard 用于复制到剪贴板功能，Vee-Validate 用于表单验证。我们通过运行以下命令来安装它们:

```
npm i axios bootstrap-vue v-clipboard vee-validate
```

安装完库之后，我们就可以开始构建应用程序了。首先，在`components`文件夹中，为我们的密码表单创建一个名为`PasswordForm.vue`的文件。然后在那里，我们加上:
...
我们在这个组件中有密码表单。该表单包括名称、URL、用户名和密码字段。都是必须的。我们使用 Vee-Validate 来验证表单字段。`ValidationObserver`组件用于验证整个表单，而`ValidationProvider`组件用于验证它包装的表单字段。

验证规则由每个字段的`rule`属性指定。我们对 URL 字段有一个特殊的`url`规则。当作用域槽中的`errors`对象具有非零长度时，我们显示验证错误消息。`state`道具用于设置验证状态，当`errors`长度为 0 时显示绿色，否则显示红色。错误信息显示在`b-form-invalid-feedback`组件中。

当用户点击保存按钮时，调用`onSubmit`函数。我们通过使用 `this.$refs.observer.validate();`获得表单的验证状态。ref 是指`ValidationObserver`的 ref。如果它解析为`true`，那么我们根据`edit`属性调用`addPassword`或`editPassword`来保存条目。然后我们通过调用`getPasswords`获得密码，然后通过调度`setPasswords`变异将密码放入我们的 Vuex 存储中。然后我们发出`saved`事件来关闭主页上的模态。

我们有一个主要在编辑现有条目时使用的`watch`块，我们获得`password`属性并通过复制属性将其设置为`this.form`，这样我们只更新`form`对象，而在绑定数据时不更新任何内容。
...
为了复制用户名和密码按钮，我们使用了`v-clipboard`指令，让我们在单击按钮时将用户名和密码分别复制到剪贴板。

在`Home.vue`中，我们将现有代码替换为:
...
在这个文件中，我们有一个表来显示密码条目的列表，并让用户打开和关闭添加/编辑模式。我们在每一行都有按钮来复制用户名和密码，还可以让用户编辑或删除每个条目。
...
这将在页面顶部添加一个引导导航条，以及一个显示我们定义的路线的`router-view`。
...
将我们安装的库添加到我们的应用程序中，以便我们可以在组件中使用它。我们在这里添加了 V-Clipboard 库，这样我们就可以在我们的主页中使用它。我们从 Vee-Validate 调用`extend`来添加我们想要使用的表单验证规则。此外，我们在这个文件中导入了引导 CSS 来获取样式。
...
这将我们的`passwords`状态添加到存储中，因此我们可以在`PasswordForm`和`HomePage`组件的`computed`块中观察到它。我们有`setPasswords`函数来更新`passwords`状态，我们通过调用`this.$store.commit(“setPasswords”, response.data);`在组件中使用它，就像我们在`PasswordForm`中做的那样。
...
![](img/1ebc9706d46d96e40ada72eea98484ad.png)![](img/5cb996c8d9117e86f16c586da2b75c5c.png)![](img/f455ed90163c9787f13d80a06ccf516f.png)![](img/3b08bd9a3f1128ce9350311d2c710546.png)